...
import { getAllCommuneAsync, setOpenAddCommune, setOpenEditCommune, setEditCommuneId } from 'apps/commune.slice'
import { CreateCommuneForm } from 'components'
...
import EditCommuneForm from 'components/editCommuneForm'
import { deleteCommuneAPI } from 'apis/communeAPI'
...
const TABLE_HEAD = ['Tên xã', 'Huyện', 'Ngày tạo', 'Ngày chỉnh sửa']

export default function CommuneListingPage() {
    ...
    const tableRow = useSelector((state: RootState) => state.communeSlice.data)
    const pagination = useSelector((state: RootState) => state.communeSlice.paginationObject)
    const handleGetAllCommunes = useCallback(
    ...
    useEffect(() => {
        dispatch(setAdminSidebarTab(9))
        handleGetAllCommunes(1)
    }, [dispatch, handleGetAllCommunes])
    ...
    const handlePageChange = (pageNumber: number) => {
        ...
        handleGetAllCommunes(pageNumber)
    }
    ...
    return (
        <>
                    ...
                    <div className='mb-4 mt-2 flex items-start justify-between gap-8'>
                        ...
                        <div className='flex shrink-0 flex-row gap-2 '>
                            <Button
                                ...
                                size='sm'
                                variant='gradient'
                                onClick={() => dispatch(setOpenAddCommune(true))}
                            >
                                                ...
                                                >
                                                    ...
                                                    <MenuList className='rounded-sm'>
                                                        <MenuItem
                                                            ...
                                                            onClick={() => {
                                                                dispatch(setEditCommuneId(item.id || ''))
                                                                dispatch(setOpenEditCommune(true))
                                                            }}
                                                        >
            ...
            <ThemeProvider value={mtTheme}>
                ...
                <ConfirmDialog
                    open={openDelete}
                    message={'Bạn có muốn xóa phường xã này không?'}
