...
import { getAllProvinceAsync, setEditProvinceData, setOpenAddProvince, setOpenEditProvince } from 'apps/province.slice'
import { showProgressLoading, hideProgressLoading } from 'apps/loading.slice'
import { useSelector } from 'react-redux'
import { CreateProvinceForm, EditProvinceForm } from 'components'
import mtTheme from 'const/MTtheme'
import { formatDisplayDate, toastError, toastSuccess } from 'utils/function'
import { deleteProvinceAPI } from 'apis/provinceAPI'
...
const TABLE_HEAD = ['Tên tỉnh', 'Quốc gia', 'Ngày tạo', 'Ngày chỉnh sửa']

export default function ProvinceListingPage() {
    ...
    const tableRow = useSelector((state: RootState) => state.provinceSlice.data)
    const pagination = useSelector((state: RootState) => state.provinceSlice.paginationObject)
    ...
    useEffect(() => {
        dispatch(setAdminSidebarTab(7))
        handleGetAllProvince(1)
    }, [dispatch, handleGetAllProvince])
    ...
    const handlePageChange = (pageNumber: number) => {
        ...
        handleGetAllProvince(pageNumber)
    }
    ...
    const handleDelete = async (id: string) => {
        ...
        if (id !== '') {
            try {
                ...
                dispatch(hideProgressLoading())
                handleGetAllProvince(pagination.currentPage)
                toastSuccess('Xóa tỉnh thành thành công')
            }
        }
    }
    return (
                    ...
                    <div className='mb-4 mt-2 flex items-start justify-between gap-8'>
                        ...
                        <div className='flex shrink-0 flex-row gap-2 '>
                            <Button
                                ...
                                size='sm'
                                variant='gradient'
                                onClick={() => dispatch(setOpenAddProvince(true))}
                            >
                                                    ...
                                                    <MenuList className='rounded-sm'>
                                                        <MenuItem
                                                            ...
                                                            onClick={() => {
                                                                ...
                                                                dispatch(setOpenEditProvince(true))
                                                            }}
                                                        >
