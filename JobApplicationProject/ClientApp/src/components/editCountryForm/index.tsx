...
import { getAllCountryAsync, setOpenEditCountry, updateCountryAsync } from 'apps/country.slice'
...
function EditCountryForm() {
    ...
    const openEditCountry = useSelector((state: RootState) => state.countrySlice.openEditCountry)
    const pageNumber = useSelector((state: RootState) => state.countrySlice.paginationObject.currentPage)
    const editCountryData = useSelector((state: RootState) => state.countrySlice.editCountry)
    const [countryName, setCountryName] = useState<string>()
    const [countryCode, setCountryCode] = useState<string>()
    const [countryISOCode, setCountryISOCode] = useState<string>()

    useEffect(() => {
        setCountryName(editCountryData.name)
        setCountryCode(editCountryData.countryCode)
        setCountryISOCode(editCountryData.countryISOCode)
    }, [editCountryData.countryCode, editCountryData.countryISOCode, editCountryData.name])

    const handleCreateCountry = async () => {
        dispatch(showProgressLoading('Đang chỉnh sửa...'))
        try {
            await dispatch(
                updateCountryAsync({
            ...
            dispatch(setOpenEditCountry(false))
            await dispatch(
                getAllCountryAsync({
    ...
    return (
        <Drawer
            ...
            className='p-4'
            onClose={() => dispatch(setOpenEditCountry(false))}
            placement='right'
        ...
        >
            <div className='mt-12'>
                <div className='mb-6 flex items-center justify-between'>
                    ...
                    <IconButton variant='text' color='blue-gray' onClick={() => dispatch(setOpenEditCountry(false))}>
                        <svg
            ...
            <div className='flex gap-2 fixed bottom-6'>
                ...
                <Button
                    size='sm'
                    variant='outlined'
                    onClick={() => dispatch(setOpenEditCountry(false))}
