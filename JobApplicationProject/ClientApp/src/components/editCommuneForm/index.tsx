...
import { getAllCommuneAsync, setOpenEditCommune, getCommuneDetailsAsync, updateCommuneAsync } from 'apps/commune.slice'
...
function EditCommuneForm() {
    ...
    const openEditCommune = useSelector((state: RootState) => state.communeSlice.openEditCommune)
    const editCommuneId = useSelector((state: RootState) => state.communeSlice.editCommuneId)
    const editCommuneData = useSelector((state: RootState) => state.communeSlice.editCommuneData)
    const getCommuneDetailsStatus = useSelector((state: RootState) => state.communeSlice.getCommuneDetailsStatus)
    ...
    const getCommuneDetails = useCallback(async () => {
        await dispatch(getCommuneDetailsAsync(editCommuneId))
    }, [dispatch, editCommuneId])
    ...
    const getDistrictByProvinceId = useCallback(
        async (id: string) => {
            await dispatch(getDistrictByProvinceIdAsync(id))
        },
        [dispatch]
    )
    const handleEditCommune = async () => {
        if (name === '') {
            ...
            try {
                ...
                dispatch(setOpenEditCommune(false))
                await dispatch(
                    getAllCommuneAsync({
    ...
    useEffect(() => {
        if (openEditCommune) getCommuneDetails()
    }, [getCommuneDetails, openEditCommune])
    useEffect(() => {
        if (getCommuneDetailsStatus === 'succeeded' && openEditCommune) {
            setName(editCommuneData.name || '')
            setProvince(editCommuneData.provinceId)
            setCountry(editCommuneData.countryId)
            setDistrict(editCommuneData.districtId)
        }
    }, [
        editCommuneData.countryId,
        editCommuneData.districtId,
        editCommuneData.name,
        editCommuneData.provinceId,
        getCommuneDetailsStatus,
        openEditCommune
    ])
    ...
    useEffect(() => {
        if (openEditCommune) getProvinceByCountryId(editCommuneData.countryId || '')
    }, [editCommuneData.countryId, getProvinceByCountryId, openEditCommune])
    useEffect(() => {
        if (openEditCommune) getDistrictByProvinceId(editCommuneData.provinceId || '')
    }, [editCommuneData.provinceId, getDistrictByProvinceId, openEditCommune])
    useEffect(() => {
    ...
    return (
        ...
        >
            ...
            <div className='flex gap-2 fixed bottom-6'>
                <Button size='sm' className='rounded-sm px-6' onClick={handleEditCommune}>
                    Lưu
                </Button>
                <Button
                    size='sm'
...
export default EditCommuneForm
